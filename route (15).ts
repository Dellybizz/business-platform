...
export async function POST(request: Request) {
  try {
    ...
    await writeAuditEvent({ workspaceId: id, actorUserId: identity.id, action: "workspace.created", targetType: "workspace", targetId: id, metadata: { type: input.type } });
...
export async function PUT(request: Request) {
  try {
    ...
    if (body.sections) await requirePermission(ctx, "pages.write");
