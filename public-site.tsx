...
export function PublicSite({ slug, pageSlug="home" }: { slug: string; pageSlug?:string }) {
  const [data, setData] = useState<{
      workspace: { name: string; type: keyof typeof copy };
  ...
  const labels = copy[data.workspace.type],
    Icon = labels.icon;
  ...
  return (
    <main className="min-h-screen bg-[#fbfaf7] text-[#191a17]">
      <header className="flex h-16 items-center justify-between border-b border-black/8 px-6 lg:px-12">
        ...
        <nav className="flex flex-wrap justify-end gap-4 text-sm text-black/50">
          {data.navigation.map((item) => <Link key={item.slug} href={`/s/${slug}${item.slug === "home" ? "" : `/${item.slug}`}`}>{item.title}</Link>)}
        </nav>
      </header>
