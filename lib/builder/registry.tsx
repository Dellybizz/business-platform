...
export function createSection(type:string,presetIndex=0):SiteSection{const definition=sectionRegistry[type];if(!definition)throw new Error(`Unknown section type: ${type}`);const preset=definition.presets[presetIndex]||definition.presets[0];return{id:uniqueId(),type,settings:{...definition.defaults,...preset.settings},blocks:preset.blocks?.map(item=>createBlock(item.type,item.preset||0))}}
