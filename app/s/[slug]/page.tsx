...
export default async function PublicSitePage({params,searchParams}:{params:Promise<{slug:string}>;searchParams:Promise<{themePreview?:string}>}){const{slug}=await params,{themePreview}=await searchParams;const workspace=await env.DB.prepare("SELECT id,theme_id AS themeId FROM workspaces WHERE slug = ?").bind(slug).first<{id:string;themeId:string}>();const presentation=workspace?await resolveThemePresentation(env.DB,{workspaceId:workspace.id,fallbackThemeId:workspace.themeId,previewToken:themePreview}):{themeId:"atelier",overrides:{}};return <div className="tenant-theme" data-theme-preview={themePreview?"true":undefined} style={themeStyle(presentation.themeId,presentation.overrides)}><PublicSite slug={slug}/></div>}
