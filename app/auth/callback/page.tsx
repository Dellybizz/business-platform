...
export default function AuthCallbackPage() {
  ...
  useEffect(() => { let active = true; void (async () => {
    try {
      ...
      const code = params.get("code"); if (!code) throw new Error("The sign-in response did not include an authorization code.");
      const supabase = getSupabaseBrowserClient();
      const { data, error: exchangeError } = await supabase.auth.exchangeCodeForSession(code);
      if (exchangeError || !data.session?.access_token) throw exchangeError || new Error("Supabase did not return a session.");
      const response = await fetch("/api/auth/supabase", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ accessToken: data.session.access_token }) });
      const payload = response.headers.get("content-type")?.includes("application/json") ? await response.json() as { error?: string } : {};
    ...
    } catch (reason) { if (active) setError(reason instanceof Error ? reason.message : "Google sign-in could not be completed."); }
  })(); return () => { active = false; }; }, []);
