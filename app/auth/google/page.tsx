...
export default function GoogleLoginStartPage() {
  ...
  useEffect(() => { void (async () => {
    try {
      ...
      const callback = new URL("/auth/callback", window.location.origin); callback.searchParams.set("returnTo", returnTo);
      const { error } = await getSupabaseBrowserClient().auth.signInWithOAuth({ provider: "google", options: { redirectTo: callback.toString(), queryParams: { prompt: "select_account" } } });
      if (error) throw error;
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Google sign-in could not be started."); }
  })(); }, []);
