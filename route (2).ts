...
export async function POST(request: Request) {
  try {
    ...
    const slugOwner = await env.DB.prepare("SELECT id FROM workspaces WHERE slug = ?").bind(input.slug).first<{ id: string }>();
  ...
  try {
    ...
    if (name || body.businessCategory !== undefined || body.themeId) {
      statements.push(env.DB.prepare(
        `UPDATE workspaces
         SET name = COALESCE(?, name), business_category = ?, theme_id = COALESCE(?, theme_id), updated_at = ?
         WHERE id = ?`,
      ).bind(name, category, body.themeId || null, now, workspaceId));
    }
